...
export default class MyTuitionsScreen extends Component {
    ...
    getaddedTuitionsList =()=>{
        if(this.state.searchText === ''){
            this.requestRef = db.collection('TuitionsList').where("userId","==",this.state.userId)
            .onSnapshot((snapshot)=>{
    ...
    getSearchList = async(text) => {
       ...
       // console.log(this.state.searchText);
        ...
        enteredText[0].toUpperCase() === "Z"){
            const search = await db.collection('TuitionsList').where("topic","==",this.state.searchText).where("userId","==",this.state.userId).get()
            search.docs.map((doc)=>{
                this.setState({
        ...
        return(
            <View style={{flex: 1}}>
              ...
              <View style={styles.subContainer}>
                <Text style={{ fontSize: 20}}> Your Tuition Offers </Text>
              </View>
                )
                :(
                <FlatList
